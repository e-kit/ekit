...
node -e "const json=require('./packages/$1/package.json');json.scripts.test='jest --all';json.scripts.build = 'tsc --project tsconfig.prod.json';json.version='0.0.1';json.main='lib/index.js';json.description = '${1}'.split('-').join(' ');json.license='MIT';require('fs').writeFileSync('./packages/$1/package.json', JSON.stringify(json, null, '  '), { encoding: 'utf8' });"
